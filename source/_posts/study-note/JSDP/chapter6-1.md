...
建構式 `Child()` 和 prototype 都是空白的，__proto__ 指向 `inherit()` 函式建立的 `new Pserson()` ，在呼叫 `say()` ，子建構式沒有這個方法，父建構式也沒有這個方法，就會再往上查找父建構式的 prototype。
...
- 第一種的預設模式使得 blog 物件可以透過原型獲得 tags 的存取權，自身不具有tags，所以是 false。
